...
struct node* search(struct node *root, int data)
{
    if(root==NULL) //if root->data is x then the element is found
        return NULL;
    else if (data>root->data) // x is greater, so we will search the right subtree
...
struct node* delete(struct node *root, int data)
{
    ...
    if(root==NULL)
     printf("No such elements Exists");
    if (data>root->data)
...
int main()
{
    ...
    */
    ...
    postorder(root);
    printf("\n");
    printf("After Deleting the Node Elements of Tree 1,40 and 45 \n");
